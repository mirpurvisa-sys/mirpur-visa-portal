...
import { delegate, formToData, parseId } from "@/lib/crud";
...
export default async function EditPage({ params }: { params: Promise<{resource:string; id:string}> }){
  ...
  const row = await delegate(resource.model).findUnique({ where: { id: parseId(id) } });
  if(!row) notFound();
  async function updateAction(formData: FormData){
    ...
    const data = formToData(res, formData); delete data.created_at;
    await delegate(res.model).update({ where: { id: parseId(id) }, data });
  ...
  async function deleteAction(){
    ...
    await delegate(res.model).delete({ where: { id: parseId(id) } });
    redirect(`/admin/${res.key}`);
  }
